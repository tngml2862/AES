...
#define _FOLDER_NAME_ "C:\Users\김한욱\source\repos\aes_cbc\aes_cbc\AES_CCM_INPUT";
#include<stdio.h>
#include<stdio.h>
#include<stdlib.h>
#include <math.h>
...
void main(void)
{
	...
	int plain_text[4][KEY_LENGHT]; /* = { {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a},
	{ 0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e, 0xb7, 0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51 },
	{ 0x30, 0xc8, 0x1c, 0x46, 0xa3, 0x5c, 0xe4, 0x11, 0xe5, 0xfb, 0xc1, 0x19, 0x1a, 0x0a,0x52, 0xef },
	{ 0xf6, 0x9f, 0x24, 0x45, 0xdf, 0x4f, 0x9b, 0x17, 0xad, 0x2b, 0x41, 0x7b, 0xe6, 0x6c, 0x37, 0x10 }
	};*/
	int IV[KEY_LENGHT]; //= { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };

	int round_arr[10][16];
	int key_s[16];
	int block[Nb][KEY_LENGHT];

	FILE *fp_key, *fp_iv, *fp_pt;
	fp_key = fopen("AES_KEY.txt", "rb");
	fp_iv = fopen("CBC_IV.txt", "rb");
	fp_pt = fopen("CBC_PT.txt", "rb");

	if (fp_key == NULL) exit(1); if (fp_iv == NULL) exit(1); if (fp_pt == NULL) exit(1);

	int data;
	for (int i = 0; i < KEY_LENGHT; i++) //파일 data값 불러오기
	{
	...
	{
		fscanf(fp_iv, "%02x", &data);
		IV[i] = (unsigned char)data;
	}
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < KEY_LENGHT; j++)
		{
			fscanf(fp_pt, "%02x", &data);
			plain_text[i][j] = (unsigned char)data;
		}
	}
	...
	for (int b = 0; b < 4; b++)  //블록 마다 암호
	{
		if (b == 0)
		{
			for (int i = 0; i < 16; i++)
			{
				block[0][i] = IV[i] ^ plain_text[b][i];  //1차 iv xor first_block
			}
		}
		else
		{
			for (int i = 0; i < 16; i++)
			{
				block[b][i] = cipher_text[b - 1][i] ^ plain_text[b][i];
			}
		}

		for (int i = 0; i < 16; i++)
		{
			aes_arr[i] = block[b][i]; //암호화할 블록 ase_arr에 복사
		}
	...
	for (i = 0; i < KEY_LENGHT; i++)
		printf("%x ", cipher_text[3][i]);
	printf("\n");

}
/*boolean plain_null_check(int plain_text[])
{
	if (plain_text == null)
	{
		return false;
...
boolean key_length_check(int key[])
{
	if (key.length == 16)
		return true;
	else
